...
const Header = ({ user, setUser }: HeaderProps) => {
    ...
    return (
        <div className="w-full mt-3 border-b-2 flex justify-between items-center py-2">
            ...
            {user.signedIn ? (
                <Link to="/login" className="text-xl px-4 py-2 border border-black mx-2">{user.username}</Link>
            ) : (
                <span className="text-xl px-4 py-2 border border-black mx-2">Guest</span>
            )}
        </div>
    );
};
